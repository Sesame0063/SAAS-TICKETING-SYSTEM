...
use crate::{
    ...
    services::{RedisService, audit::AuditService, email::EmailService},
...
impl TicketService {
    ...
    pub async fn get_ticket_by_id(
        ...
        if let Ok(Some(ticket)) =
            RedisService::get::<TicketResponse>(redis, &cache_key).await
        {
            tracing::debug!(
                ticket_id = %ticket_id,
                "Redis cache hit"
    ...
    pub async fn update_ticket(
        ...
        let ticket = ticket_repository::update(
            pool,
            ticket_id,
            customer_id,
            request,
        ...
        if let Err(err) =
            RedisService::delete(redis, &ticket_cache_key(ticket.id)).await
        {
            tracing::warn!(
                ticket_id = %ticket.id,
                error = ?err,
        ...
        ws.broadcast_to_room(
            &ticket.id,
            WsEvents::ticket_updated(ticket.id),
        )
        .await;
    ...
    pub async fn delete_ticket(
        ...
        let deleted =
            ticket_repository::delete(pool, ticket_id, customer_id).await?;
        ...
        if let Err(err) =
            RedisService::delete(redis, &ticket_cache_key(ticket_id)).await
        {
            tracing::warn!(
                ticket_id = %ticket_id,
                error = ?err,
    ...
    pub async fn assign_ticket(
        ...
        let ticket = ticket_repository::assign(
            pool,
            ticket_id,
            request.agent_id,
        )
        ...
        if let Ok(Some(agent)) =
            UserRepository::find_by_id(pool, request.agent_id).await
        {
    ...
    pub async fn update_status(
        ...
        let ticket = ticket_repository::update_status(
            pool,
            ticket_id,
        ...
        if ticket.status == "Closed" || ticket.status == "Resolved" {
            if let Ok(Some(customer)) =
                UserRepository::find_by_id(pool, ticket.customer_id).await
            {
        ...
        ws.broadcast_to_room(
            &ticket.id,
            WsEvents::ticket_status_updated(
                ticket.id,
                request.status.clone(),
            ),
        )
